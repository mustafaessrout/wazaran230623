...
public partial class fm_salesreceipt2_uom : System.Web.UI.Page
{
    cbll bll = new cbll();
    ...
    double dQtyFreeCtn = 0, dQtyFreePcs = 0, dQtyFreeTotal = 0;
    double dQtyFreeShipCtn = 0, dQtyFreeShipPcs = 0, dQtyFreePcsTotal = 0;

    double dOrder = 0; double dShip = 0; double dConv = 0; double dConvShip = 0;
    double dOrderCtn = 0, dOrderPcs = 0, dOrderTotal = 0;
    double dShipCtn = 0, dShipPcs = 0, dShipTotal = 0;
    ...
    void vInitGrid()
    {
        try
        {
            dOrder = 0; dShip = 0;  dConv = 0;  dConvShip = 0;
            dOrderCtn = 0; dOrderPcs = 0; dOrderTotal = 0;
            dShipCtn = 0;  dShipPcs = 0; dShipTotal = 0;
            List<cArrayList> arr = new List<cArrayList>();
            //arr.Add(new cArrayList("@inv_no", hdinv.Value.ToString()));
            //arr.Add(new cArrayList("@usr_id", Request.Cookies["usr_id"].Value.ToString()));
            //bll.vInsertWrkDosalesDtl(arr);
            ...
            foreach (GridViewRow row in grd.Rows)
            {
                ...
                else
                {
                    ...
                    HiddenField lbqtyshipment = (HiddenField)row.FindControl("lbqtyshipment");
                    HiddenField hdqtyshipment_ctn = (HiddenField)row.FindControl("hdqtyshipment_ctn");
                    HiddenField hdqtyshipment_pcs = (HiddenField)row.FindControl("hdqtyshipment_pcs");
                    HiddenField hdqtyconv_ctn = (HiddenField)row.FindControl("hdqtyconv_ctn");

                    dOrderCtn = dOrderCtn + double.Parse(hdqty_ctn.Value);
                    dOrderPcs = dOrderPcs + double.Parse(hdqty_pcs.Value);
                    dShipCtn = dShipCtn + double.Parse(hdqtyshipment_ctn.Value);
                    dShipPcs = dShipPcs + double.Parse(hdqtyshipment_pcs.Value);
                    dOrder = dOrder + double.Parse(lbqty.Value);
                    dShip = dShip + double.Parse(lbqtyshipment.Value);
                    dConvShip = dConvShip + double.Parse(hdqtyconv_ctn.Value);
            ...
            HiddenField hdtotqtyorder = (HiddenField)grd.FooterRow.FindControl("hdtotqtyorder");
            HiddenField hdtotqtyshipment = (HiddenField)grd.FooterRow.FindControl("hdtotqtyshipment");

            lbqtotqtyorder.Text = dOrderCtn.ToString() + " CTN, " + dOrderPcs.ToString() + " PCS";
            lbtotqtyshipment.Text = dShipCtn.ToString() + " CTN, " + dShipPcs.ToString() + " PCS";
            hdtotqtyorder.Value = dOrder.ToString(); hdtotqtyshipment.Value = dShip.ToString();
            ...
            dQtyFree = 0; dQtyFreeShip = 0;
            dQtyFreeCtn = 0; dQtyFreePcs = 0; dQtyFreeTotal = 0;
            dQtyFreeShipCtn = 0; dQtyFreeShipPcs = 0; dQtyFreePcsTotal = 0;
            if (grdfree.Rows.Count > 0)
            {
                ...
                HiddenField hdtotreceived = (HiddenField)grdfree.FooterRow.FindControl("hdtotreceived");
                ...
                foreach (GridViewRow row in grdfree.Rows)
                {
                    ...
                    else
                    {
                        ...
                        Label lbitemcode = (Label)row.FindControl("lbitemcode");
                        HiddenField lbqtyfree = (HiddenField)row.FindControl("lbqtyfree");
                        HiddenField hdqtyfree_ctn = (HiddenField)row.FindControl("hdqtyfree_ctn");
                        HiddenField hdqtyfree_pcs = (HiddenField)row.FindControl("hdqtyfree_pcs");
                        HiddenField hdqtyconv_ctn = (HiddenField)row.FindControl("hdqtyconv_ctn");
                        ...
                        dQtyFreeCtn = dQtyFreeCtn + double.Parse(hdqtyfree_ctn.Value);
                        dQtyFreePcs = dQtyFreePcs + double.Parse(hdqtyfree_pcs.Value);
                        dQtyFree = dQtyFree + double.Parse(lbqtyfree.Value);
                        dConvShip = dConvShip + double.Parse(hdqtyconv_ctn.Value);
                    }
                }
                //lbtotqtyfree.Text = dQtyFree.ToString();
                //lbtotreceived.Text = dQtyFreeShip.ToString();
                lbtotqtyfree.Text = dQtyFreeCtn.ToString() + " CTN, " + dQtyFreePcs.ToString() + " PCS";
                lbtotreceived.Text = dQtyFreeCtn.ToString() + " CTN, " + dQtyFreePcs.ToString() + " PCS";
                hdtotreceived.Value = dQtyFree.ToString();
    ...
    protected void btinv_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (grdfree.Rows.Count > 0)
            {
                //Label lbtotreceived = (Label)grdfree.FooterRow.FindControl("lbtotreceived");
                HiddenField hdtotreceived = (HiddenField)grdfree.FooterRow.FindControl("hdtotreceived");
                dTotReceived = Double.Parse(hdtotreceived.Value);
            ...
            //double dReceived = Double.Parse(hdtotqtyshipment.Value) + dTotReceived;
            //totDelivered = Double.Parse(hdtotqtyshipment.Value) + dTotReceived;
            //totReceived = Double.Parse(hdtotqtyshipment.Value) + dTotReceived;
            //lbdelivery.Text = dReceived.ToString();
            lbdelivery.Text = (dShipCtn+dQtyFreeCtn).ToString() + " CTN, " + (dShipPcs+dQtyFreePcs).ToString() + " PCS";

            // Add Driver 
            arr.Clear();
            arr.Add(new cArrayList("@inv_no", hdinv.Value.ToString()));
            arr.Add(new cArrayList("@usr_id", Request.Cookies["usr_id"].Value.ToString()));
            arr.Add(new cArrayList("@qty", dConvShip.ToString()));
            arr.Add(new cArrayList("@salespointcd", Request.Cookies["sp"].Value.ToString()));
            ...
            foreach (GridViewRow row in grddriver.Rows)
            {
                ...
                HiddenField hdqtydriver = (HiddenField)row.FindControl("hdqtydriver");
                dBring += Double.Parse(hdqtydriver.Value);
                lbqtydriver.Text = lbdelivery.Text;
            }
            if (grddriver.Rows.Count > 0)
            {
                Label lbtotqty2 = (Label)grddriver.FooterRow.FindControl("lbtotqty");
                HiddenField hdqtytot2 = (HiddenField)grddriver.FooterRow.FindControl("hdtotqty");
                //lbtotqty2.Text = dBring.ToString();
                hdqtytot2.Value = dBring.ToString();
                lbtotqty2.Text = lbdelivery.Text;
            }
            ...
            if (statusInvoice == "D")
            {
                ...
                grdfree.Columns[6].Visible = false;
