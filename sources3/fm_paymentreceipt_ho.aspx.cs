...
public partial class fm_paymentreceipt_ho : System.Web.UI.Page
{
    cbll bll = new cbll();
    cdal cdl = new cdal();
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            try
            {
                ...
                bll.vDelWrkPaymentInvoiceHO(arr);
                bll.vDeletePaymentWrkSalespoint(arr);
            ...
            catch (Exception ex)
            {
                ...
                bll.vHandledError(ref ex, Request.Cookies["sp"].Value.ToString()+" : fm_paymentreceipt_ho");
    ...
    void BindingGrid()
    {
        try
        {
            ...
            arr.Add(new cArrayList("@usr_id", Request.Cookies["usr_id"].Value.ToString()));
            arr.Add(new cArrayList("@salespointcd", Request.Cookies["sp"].Value.ToString()));
            bll.vBindingGridToSp(ref grd, "sp_twrk_paymentinvoice_ho_get", arr);
        ...
        try
        {
            foreach (GridViewRow row in grd.Rows)
            {
                if ((row.RowType == DataControlRowType.DataRow) && (row.RowState == DataControlRowState.Normal || row.RowState == DataControlRowState.Alternate))
                {
                    ...
                    if (chk.Checked)
                    {
                        string sCheck1pct = bll.vLookUp("select dbo.fn_checkdiscountpayment1pct('" + lbinvoiceno.Text + "','" + hdsalespoint.Value.ToString() + "')");
                        if (sCheck1pct == "ok")
                        {
                            chk.Checked = false;
    ...
    protected void grd_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        try
        {
            ...
            {
                HiddenField hdsalespoint = (HiddenField)e.Row.FindControl("hdsalespoint");
                ...
                string sInv = bll.vLookUp("select dbo.fn_checkinvreceived('" + lbinvoiceno.Text + "','" + hdsalespoint.Value.ToString() + "')");
                if (sInv != "ok")
                {
    ...
    protected void btsave_Click(object sender, EventArgs e)
    {
        ...
        {
            ScriptManager.RegisterStartupScript(Page, Page.GetType(), "al", "sweetAlert('Total Amount must be numeric','Check Total Amount','warning');", true);
            return;
        }
        ...
        if (grd.Rows.Count > 0)
        {
            ...
            lbtotamt.Text = bll.vLookUp("select sum(amt) from twrk_paymentinvoice where usr_id='" + Request.Cookies["usr_id"].Value.ToString() + "'");
            lbtotRound.Text = bll.vLookUp("select sum(round_amt) from twrk_paymentinvoice where usr_id='" + Request.Cookies["usr_id"].Value.ToString() + "'");
            lbtotdisc.Text = bll.vLookUp("select sum(disc_amt) from twrk_paymentinvoice where usr_id='" + Request.Cookies["usr_id"].Value.ToString() + "'");
            lbtotOutStand.Text = bll.vLookUp("select sum(outstanding) from twrk_paymentinvoice where usr_id='" + Request.Cookies["usr_id"].Value.ToString() + "'");
        }
        ...
        arrCheck.Add(new cArrayList("@usr_id", Request.Cookies["usr_id"].Value.ToString()));
        dtpaymentinvoice = cdl.GetValueFromSP("sp_tdosales_invoice_searchWithInv2", arrCheck);

        string ka = bll.vLookUp("select otlcd from tmst_customer where cust_cd='" + hdcust.Value.ToString() + "'"); ;
        int cnt = 0;

        foreach (DataRow dr in dtpaymentinvoice.Rows)
        {
            string balance = bll.vLookUp("select dbo.fn_gettempbalance('" + dr["inv_no"].ToString() + "',null)");
        ...
        if (!chAdvance.Checked)
        {
            foreach (GridViewRow row in grd.Rows)
            {
                if (row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    string sMsg = bll.vLookUp("select dbo.fn_checkcndn('" + lbinvoiceno.Text + "','" + hdsalespoint.Value.ToString() + "')");
                    if ((sMsg != "ok") && (dCheck > 0))
                    {
        ...
        List<cArrayList> arr = new List<cArrayList>();
            ...
            bll.vInsertMstPaymentHO(arr, ref sPaymentNo);


            txreceiptno.Text = sPaymentNo;
            hdpaid.Value = sPaymentNo;

            if (fup.PostedFile.FileName != "")
            {
                FileInfo fi = new FileInfo(fup.FileName);
                fup.SaveAs(bll.sGetControlParameter("image_path") + "payment_ho\\" + sPaymentNo + fi.Extension);
                arr.Clear();
                arr.Add(new cArrayList("@salespointcd", Request.Cookies["sp"].Value.ToString()));
                arr.Add(new cArrayList("@payment_no", sPaymentNo));
                arr.Add(new cArrayList("@docfile", sPaymentNo + fi.Extension));
                if ((cbpaymnttype.SelectedValue.ToString() == "BT" || (cbpaymnttype.SelectedValue.ToString() == "CQ")))
                {
            ...
            if (!chAdvance.Checked)
            {
                foreach (GridViewRow row in grd.Rows)
                {
                    if (row.RowType == DataControlRowType.DataRow)
                    {
                        ...
                        {
                            bll.vInsertPaymentDtl1pctDiscHO(arr);
                        }
                        else
                        {
                            bll.vInsertPaymentBooking1pctDiscHO(arr);
                        }
                        //----------------------------- IA 21-Sep-2016-----------
                        arr.Clear();
                        arr.Add(new cArrayList("@inv_no", lbinvoiceno.Text));
                        ...
                        string sLasPromised = bll.vLookUp("select dbo.fn_getlastpromised('" + hdcust.Value.ToString() + "','"+ hdsalespoint.Value.ToString() + "')");
            ...
            // Only For Inv
            //ScriptManager.RegisterStartupScript(Page, Page.GetType(), "pr", "openreport('fm_paymentreceipt1PctDiscLookUp.aspx?cust_cd=" + hdcust.Value + "&cusgrcd=" + cbgroup.SelectedValue.ToString() + "');", true);
            ScriptManager.RegisterStartupScript(Page, Page.GetType(), Guid.NewGuid().ToString(), "sweetAlert('Payment has been saved succesfully!','Payment No. : " + sPaymentNo + "','success');", true);
        }
        catch (Exception ex)
        {
            Session["error"] = "500" + " - " + bll.sGetFieldValue("error", "500");
    ...
    protected void cbpaymnttype_SelectedIndexChanged(object sender, EventArgs e)
    {
        try
        {
            ...
            else if (cbpaymnttype.SelectedValue.ToString() == "CH")
            {
                txdocno.Text = "";
    ...
    {
        //ScriptManager.RegisterStartupScript(Page, Page.GetType(), "opr", "popupwindow('fm_lookuppaymenttab.aspx?pt=" + cbpaymnttype.SelectedValue.ToString() + "');", true);
    }
    protected void btreceiptno_Click(object sender, EventArgs e)
    {
        ScriptManager.RegisterStartupScript(Page, Page.GetType(), "op", "popupwindow('lookpayment_ho.aspx?pt=" + cbpaymnttype.SelectedValue.ToString() + "');", true);
    }
    protected void btprint_Click(object sender, EventArgs e)
    {
        try
        {
            string sSta = bll.vLookUp("select payment_sta_id from tmst_payment_ho where payment_no='" + txreceiptno.Text + "'");
            ...
            {
                ScriptManager.RegisterStartupScript(Page, Page.GetType(), "pr", "openreport('fm_report2.aspx?src=pychequeho&py=" + txreceiptno.Text + "');", true);
            }
            else
            {
                ScriptManager.RegisterStartupScript(Page, Page.GetType(), "pr", "openreport('fm_report2.aspx?src=pyho&noy=" + txreceiptno.Text + "');", true);
            }
    ...
    protected void cbsource_SelectedIndexChanged(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                dtpayment.CssClass = "form-control";
                txmanualno.CssClass = "form-control";
                txdocno.CssClass = "form-control ro";
                //txtabno.CssClass = "form-control ro";
                txrenark.CssClass = "form-control";
                dtcheque.CssClass = "form-control ro";
                txdocno.CssClass = "form-control ro";
                dtdue.CssClass = "form-control ro";
                txreceiptno.CssClass = "form-control ro";
                btsave.CssClass = "btn btn-warning";
                rdpaid.CssClass = "form-control";
                //    bttabno.CssClass = "divhid";
                //     txtabnoPnl.CssClass = "";
                //     bttabnoPnl.CssClass = "";
                //    btreceiptno.CssClass = "btn btn-primary";
                txreceiptnoPnl.CssClass = "input-group";
                //     btreceiptnoPnl.CssClass = "input-group-btn";
                txdocno.Text = "";
                cbpaymnttype.CssClass = cd.csstext;
            ...
            else
            {
                ...
                btsave.CssClass = "divhid";
                rdpaid.CssClass = "form-control ro";
                //bttabno.CssClass = "btn btn-primary";
                //   txtabnoPnl.CssClass = "input-group";
                //   bttabnoPnl.CssClass = "input-group-btn";
                // btreceiptno.CssClass = "divhid";
                //   txreceiptnoPnl.CssClass = "";
                // btreceiptnoPnl.CssClass = "";
    ...
    {
        Response.Redirect("fm_paymentreceipt_ho.aspx");
    }
    ...
    public static string[] GetCompletionList2(string prefixText, int count, string contextKey)
    {
        ...
        //arr.Add(new cArrayList("@salespointcd", cok.Value.ToString()));
        bll.vSearchInvoiceByTypeAll(arr, ref rs);
        //DataTable dtTemp = new DataTable();
        //dtTemp = cdl.GetValueFromSP("sp_twrk_paymentinvoice_get", arr);

        while (rs.Read())
        {
            string balance = bll.vLookUp("select dbo.fn_gettempbalance('" + rs["inv_no"].ToString() + "','"+ rs["salespointcd"].ToString() + "')");

            if (balance != "")
            {
                if (Convert.ToDecimal(balance) > 0)
                {
                    sItem = AjaxControlToolkit.AutoCompleteExtender.CreateAutoCompleteItem(rs["salespoint_nm"].ToString() + " | " + rs["inv_no"].ToString() + " | Cust-" + rs["cust_cd"].ToString() + " | " + " Inv_dt-" + Convert.ToDateTime(rs["inv_dt"]).ToString("dd-MM-yyyy") + " | Balance - " + balance + " ",
                        rs["inv_no"].ToString());
                    lItem.Add(sItem);
                }
            }
    ...
    protected void btsearchinv_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            //arr.Add(new cArrayList("@salespointcd", Request.Cookies["sp"].Value.ToString()));
            bll.vGetDOSalesInvoiceByStatus2(arr, ref rs);
            lbalert.ForeColor = System.Drawing.Color.Red;
            lbalert.Font.Bold = true; ;
            chonepct.Enabled = false;
            while (rs.Read())
            {
                txsearchinv.Text = rs["inv_no"].ToString();
                ...
                lbhaspaid.Text = bll.vLookUp("select isnull(sum(amt),0) from tpayment_dtl d join tmst_payment m on d.payment_no=m.payment_no and d.salespointcd=m.salespointcd where d.inv_no='" + hdinv.Value.ToString() + "' and m.payment_sta_id <> 'L' and d.salespointcd='" + rs["salespointcd"].ToString() + "'");
                lbtempbalance.Text = bll.vLookUp("select dbo.fn_gettempbalance('" + hdinv.Value.ToString() + "','" + rs["salespointcd"].ToString() + "')");
                string sDisc1pct = bll.vLookUp("select dbo.fn_checkdiscountpayment1pct('" + hdinv.Value.ToString() + "','" + rs["salespointcd"].ToString() + "')");
    ...
    protected void btadd_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            //DateTime dt = DateTime.ParseExact(lbinvoicedate.Text, "d/M/yyyy", System.Globalization.CultureInfo.InvariantCulture);
            //string sdate;
            ...
            if (grd.Rows.Count > 0)
            {
                ...
                arr.Add(new cArrayList("@usr_id", Request.Cookies["usr_id"].Value.ToString()));
                arr.Add(new cArrayList("@salespointcd", Request.Cookies["sp"].Value.ToString()));
                dtpaymentinvoice = cdl.GetValueFromSP("sp_twrk_paymentinvoice_ho_get", arr);
    ...
    protected void btaddsalespoint_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            else
            {
                ...
                bll.vInsertPaymentWrkSalespoint(arr); arr.Clear();
                arr.Add(new cArrayList("@usr_id", Request.Cookies["usr_id"].Value.ToString()));
                bll.vBindingGridToSp(ref grdslspoint, "sp_twrk_paymentsalespoint_get", arr);
