...
public partial class fm_mstrps : System.Web.UI.Page
{
    cbll bll = new cbll();
    ...
    protected void btadd_Click(object sender, EventArgs e)
    {
        try
        {
            if (rdby.SelectedIndex == 0)
            {
                ...
                string sCheckrpsbydate = bll.vLookUp("select dbo.fn_checkbydate('" + cbsalesman.SelectedValue + "','" + hdcust.Value + "','" + Request.Cookies["sp"].Value.ToString() + "')");
            ...
            else
            {
                ...
                string sCheckrpsbyday = bll.vLookUp("select dbo.fn_checkrpsbyday('" + cbsalesman.SelectedValue + "','" + hdcust.Value + "','" + Request.Cookies["sp"].Value.ToString() + "')");
    ...
    protected void grdbydate_RowDeleting(object sender, GridViewDeleteEventArgs e)
    {
        try
        {

            Label lbcustcode = (Label)grdbydate.Rows[e.RowIndex].FindControl("lbcustcode");
            List<cArrayList> arr = new List<cArrayList>();
            arr.Add(new cArrayList("@usr_id", Request.Cookies["usr_id"].Value.ToString()));
            arr.Add(new cArrayList("@cust_cd", lbcustcode.Text));
            arr.Add(new cArrayList("@salespointcd", Request.Cookies["sp"].Value.ToString()));
    ...
    {
        ScriptManager.RegisterStartupScript(Page, Page.GetType(), "op", "openreport('fm_report2.aspx?src=rp&da=" + cbdaycode.SelectedValue.ToString() + "&emp="+cbsalesman.SelectedValue+"');",true);
    }
