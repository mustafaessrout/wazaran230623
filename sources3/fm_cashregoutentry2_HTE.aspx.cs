...
public partial class fm_cashregoutentry2_HTE : System.Web.UI.Page
{
    cbll bll = new cbll();
    string cbType = null;
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            hdemp.Value = bll.vLookUp("select emp_cd from tmst_employee where emp_nm='OFFICE'");
        }
        //cbinout.Items.Remove(cbinout.Items.FindByValue("O"));
        //cbinout.Items.Insert(0, new ListItem("HO To Employee", "O"));
        ...
        cbroutine.Items.Remove(cbroutine.Items.FindByValue("R"));
        cbroutine.Items.Remove(cbroutine.Items.FindByValue("0"));
        ...
        string CommandText2 = "select * from tfield_value where fld_nm='cashout_typ' and fld_valu not in('ADV');";
        ...
        while (rdr2.Read())
        {
            string itemToRmv2 = rdr2["fld_valu"].ToString();

            cbcategory.Items.Remove(cbcategory.Items.FindByValue(itemToRmv2));
        }
        //con2.Close();
        ...
        //List<cArrayList> arr = new List<cArrayList>();
        //arr.Add(new cArrayList("@cashout_typ", "ADV"));
        //arr.Add(new cArrayList("@routine", "NR"));
        ...
        txpic.Text = "0001-OFFICE";
        txpic.Enabled = false;
        cbdept.SelectedValue = "FI";
        cbdept.Enabled = false;

        if (cbcategory.SelectedValue == "ADV")
        {
            cbitem.Items.Clear();
            cbitem.Items.Insert(0, new ListItem("EMPLOYEE ADVANCE", "CHA00048"));
            cbitem.SelectedValue = "CHA00048";
        }
    }
    protected void cbcategory_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        if (cbinout.SelectedValue.ToString() == "O" || cbinout.SelectedValue.ToString() == "O")
        {
            List<cArrayList> arr = new List<cArrayList>();
    ...
    protected void cbroutine_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        if (cbroutine.SelectedValue.ToString() == "NR")
        {
            ...
            if (cbcategory != null && cbinout.SelectedValue.ToString() != "O")
            {
                cbcategory.Items.FindByValue("ADV").Attributes.Add("style", "background-color:#ADFF2F");
            }
    ...
    protected void grdattr_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        else if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            //    }
            //    else if (lbattribute.Text == "Emp Name")
            //    {
    ...
    protected void btsave_Click(object sender, EventArgs e)
    {
        ...
        if (txmanualno.Text == "")
        {
            ScriptManager.RegisterStartupScript(Page, Page.GetType(), Guid.NewGuid().ToString(), "sweetAlert('Manual No can empty!','Manual No','warning');", true);
            return;
        }
        if (txremark.Text == "")
        {
            ScriptManager.RegisterStartupScript(Page, Page.GetType(), Guid.NewGuid().ToString(), "sweetAlert('Remark can empty!','Remark','warning');", true);
            return;
        }
        if (cbinout.SelectedValue != "I" && cbroutine.SelectedValue != "R" && cbcategory.SelectedValue != "HEADOFFICE" && cbitem.SelectedValue != "CHA00142")
        {
            if (hdemp.Value.ToString() == "")
            {
                ScriptManager.RegisterStartupScript(Page, Page.GetType(), Guid.NewGuid().ToString(), "sweetAlert('PIC must selected!','PIC','warning');", true);
                return;
            }
        }
        if (!double.TryParse(txamt.Text, out dAmt))
        {
            ScriptManager.RegisterStartupScript(Page, Page.GetType(), Guid.NewGuid().ToString(), "sweetAlert('Amout can not empty!','Amout Cashout','warning');", true);
            return;
        }

        if (cbitem.SelectedValue.ToString() == "")
        {
            ScriptManager.RegisterStartupScript(Page, Page.GetType(), Guid.NewGuid().ToString(), "sweetAlert('Item cashout must be select!','Select Item Cashout','warning');", true);
            return;
        }
        ...
        {
            ScriptManager.RegisterStartupScript(Page, Page.GetType(), Guid.NewGuid().ToString(), "sweetAlert('Transaction Document must be uploaded!','Non Routine','warning');", true);
            return;
        }
        ...
        //        lbvalue.Text = bll.vLookUp("select qry_data from tmap_query where qry_cd='EMPOffice'").ToString();
        //        if (lbvalue.Text == "" || lbvalue.Text == null)
        //        {
        //            ScriptManager.RegisterStartupScript(Page, Page.GetType(), Guid.NewGuid().ToString(), "sweetAlert('Additional information for cashout is not completeld!','Additional Info','warning');", true);
        //            return;
        //        }
        ...
        //lbstatus.Text = bll.vLookUp("select fld_desc from tfield_value where fld_nm='cashout_sta_id' and fld_valu=(select cashout_sta_id from tcashout_request where cashout_cd='" + sCashout + "')");
    ...
    {
        Response.Redirect("fm_cashregoutentry2_HTE.aspx");
    }
    ...
    protected void grdattr_RowUpdating(object sender, GridViewUpdateEventArgs e)
    {
        ...
        grdattr.EditIndex = -1;
        vInitGrid();
        if (cbitem.SelectedValue.Equals("CHA00048"))
        {
    ...
    public void btsysno_Click(object sender, EventArgs e)
    {
        ...
        ScriptManager.RegisterStartupScript(Page, this.GetType(), "CallMyFunction", "PopupCenter('lookupcashout2.aspx', 'xtf', '900', '500');", true);
