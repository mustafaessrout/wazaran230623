...
public partial class fm_closingdaily : System.Web.UI.Page
{
    cbll bll = new cbll();
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            catch (Exception ex)
            {
                ...
                bll.vHandledError(ref ex, Request.Cookies["sp"].Value.ToString()+" : fm_closingdaily");
    ...
    protected void btstart_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            else
            {
                ...
                if (sBatctToday != "ok")
                {
                    ScriptManager.RegisterStartupScript(Page, Page.GetType(), Guid.NewGuid().ToString(), "sweetAlert('Closing can not done with same calendar date as today!','System date can not same with calendar date','warning');", true);
                    ScriptManager.RegisterStartupScript(Page, Page.GetType(), Guid.NewGuid().ToString(), "vDisableShow();", true);
                    return;
                }
                ...
                chclaim.Checked = true;
                chclaim.ForeColor = System.Drawing.Color.Blue;
                // System.Threading.Thread.Sleep(1000);
                ...
                //bll.vBatchBackupDBAfter(); by yanto 11-01-2018
        ...
        ScriptManager.RegisterStartupScript(Page, Page.GetType(), Guid.NewGuid().ToString(), "sweetAlert('Daily Closed Successfully','New Open Date " + sDate + "','success')", true);
