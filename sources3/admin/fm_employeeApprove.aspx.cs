...
public partial class fm_employeeApprove : System.Web.UI.Page
{
    cbll bll = new cbll();
    ...
    public void BindCotrole(string emp_cd)
    {
        try
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                if ((Convert.ToString(dt.Rows[0]["haveFamily"])) == "True")
                {
                    chfam.Checked = true;
                    txtDependant.Text = Convert.ToString(dt.Rows[0]["dependantsNo"]);
                    Depend.Visible = true;
                }
                else
                {
                ...
                txtNameArabic.ToolTip = txtNameArabic.Text;
                ...
                txtemployee_app_cd.Text = Convert.ToString(dt.Rows[0]["employee_app_cd"]);
                lblStatus.Text = Convert.ToString(dt.Rows[0]["app_sta"]);
                if (lblStatus.Text == "Approved")
                {
                    lblStatus.ForeColor = System.Drawing.Color.Green;
                }
                if (lblStatus.Text == "Partially Approved")
                {
                    lblStatus.ForeColor = System.Drawing.Color.Green;
                }
    ...
    protected void btsave_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            arr.Add(new cArrayList("@employee_app_sta", "PA"));
            ...
            ScriptManager.RegisterStartupScript(Page, Page.GetType(), "alrt", "sweetAlert('Employee has been Partially Approved successfully!','" + ddlEmployee.SelectedItem.Text + "','success');", true);

            // here re bind
            DataTable dt = new DataTable();
            List<cArrayList> arr1 = new List<cArrayList>();
            arr1.Add(new cArrayList("@salespointcd", cbAllBranch.SelectedValue));
            dt = cdl.GetValueFromSP("Sp_tmst_employeeByAll_get", arr1);
            ddlEmployee.DataSource = dt;
            ddlEmployee.DataValueField = "emp_cd";
            ddlEmployee.DataTextField = "fullname";
            ddlEmployee.DataBind();
        ...
        {
            ut.Logs("", "HR", "Employee", "fm_employeeApprove", "btsave_click", "Exception", ex.Message + ex.InnerException);
            ScriptManager.RegisterStartupScript(Page, Page.GetType(), "al", "sweetAlert('" + ex.Message + ex.InnerException + "','Error to save','warning');", true);
            //ScriptManager.RegisterStartupScript(Page, Page.GetType(), Guid.NewGuid().ToString(), "MsgWarning('File upload failed!','File upload failed');", true);
    ...
    protected void btReject_Click(object sender, EventArgs e)
    {
        ...
        //arr.Add(new cArrayList("@cust_nm", hdEmployee.Value));
        ScriptManager.RegisterStartupScript(Page, Page.GetType(), "alrt", "sweetAlert('Employee has been Rejected successfully!','" + ddlEmployee.SelectedItem.Text + "','success');", true);
        DataTable dt = new DataTable();
        List<cArrayList> arr1 = new List<cArrayList>();
        arr1.Add(new cArrayList("@salespointcd", cbAllBranch.SelectedValue));
        dt = cdl.GetValueFromSP("Sp_tmst_employeeByAll_get", arr1);
    ...
    protected void chfam_CheckedChanged(object sender, EventArgs e)
    {
        if (chfam.Checked == true)
        {
            Depend.Visible = true;
        }
        else
    ...
    protected void btprintEmpAll_Click(object sender, EventArgs e)
    {
        ScriptManager.RegisterStartupScript(Page, Page.GetType(), Guid.NewGuid().ToString(), "openreport('../fm_report2.aspx?src=employeeAppAll');", true);
    }
    protected void btprintEmpAllbyBrn_Click(object sender, EventArgs e)
    {

        if (cbAllBranch.SelectedIndex == 0)
        {
            ScriptManager.RegisterStartupScript(Page, Page.GetType(), Guid.NewGuid().ToString(), "sweetAlert('Cannot Print Report!','Branch is not Selected','warning');", true);
            return;
        }
        ScriptManager.RegisterStartupScript(Page, Page.GetType(), Guid.NewGuid().ToString(), "openreport('../fm_report2.aspx?src=employeeAppAllbrn&sp=" + cbAllBranch.SelectedValue + "');", true);
    }
}