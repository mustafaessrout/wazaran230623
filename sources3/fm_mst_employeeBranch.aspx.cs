...
public partial class fm_mst_employeeBranch : System.Web.UI.Page
{
    //Can not delete
    //Can not new
    ...
    private void bindControl()
    {
        try
        {
            ...
            //bll.vBindingComboToSp(ref ddlEmployee, "Sp_tmst_employeeBySal_get", "emp_cd", "fullname");
            ...
            dt = cdl.GetValueFromSP("Sp_tmst_employeeBySal_get", arr);
        ...
        {
            ut.Logs("", "HR", "Employee", "fr_mst_employee", "BindControl", "Exception", ex.Message + ex.InnerException);
    ...
    protected void btsave_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            string cnt = bll.vLookUp("select count(*) from tmst_employee_app where emp_cd='" + ddlEmployee.SelectedValue + "' and employee_app_sta='N'");
            ...
            bll.vInsert_tmst_employee_app(arr, ref employee_app_cd);
            ...
            else { license_doc = hdfDrivingLicense.Value; }
            ...
            else { emp_status_photo = hdfEmp_std.Value; }
            if (uplIqama.HasFile)
            {
            ...
            string sSubject = "New Employee infromation for updated Request"; string sMessage = " employee :" + ddlEmployee.SelectedItem.Text + " has been created ";
            ...
            sSubject = "New Employee infromation update Branch " + cbsalespoint.SelectedItem.Text + " , employee :" + ddlEmployee.SelectedItem.Text + " has been created ";
            ...
            ClearControl();
            bindControl();
            ScriptManager.RegisterStartupScript(Page, Page.GetType(), Guid.NewGuid().ToString(), "sweetAlert('Data Save Successfully. " + employee_app_cd + "','Data Save Successfully.','success');", true);

        }
        catch (Exception ex)
        {
            ut.Logs("", "HR", "Employee", "fr_mst_employee", "btnSave_Click", "Exception", ex.Message + ex.InnerException);
            ScriptManager.RegisterStartupScript(Page, Page.GetType(), "al", "sweetAlert('" + ex.Message + ex.InnerException + "','Error to save','warning');", true);
            //ScriptManager.RegisterStartupScript(Page, Page.GetType(), Guid.NewGuid().ToString(), "MsgWarning('File upload failed!','File upload failed');", true);
    ...
    public void BindCotrole(string emp_cd)
    {
        try
        {
            ...
            arr.Add(new cArrayList("@emp_cd", Convert.ToString(ddlEmployee.SelectedValue)));
            bll.vBindingGridToSp(ref grdVehicle, "sp_tmst_vehicleByEmp_get", arr);
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                txtVehicleSponsor.Text = Convert.ToString(dt.Rows[0]["owner_id"]);
                txtVIN.Text = Convert.ToString(dt.Rows[0]["series_haikal"]);
                ...
                txtModelYear.Text = Convert.ToString(dt.Rows[0]["model_year"]);
                ...
                txtCarColor.Text = Convert.ToString(dt.Rows[0]["color"]);
                ...
                string sUrl = "http://172.16.1.18/";//bll.vLookUp("select dbo.fn_getcontrolparameter('link_branch')");
                ...
                if (dt.Rows[0]["exp_dt"].ToString() == "")
                {
                    txtCarExpiry.Text = System.DateTime.Today.ToString("d/M/yyyy");
                }
                else
                {
                    txtCarExpiry.Text = Convert.ToDateTime(dt.Rows[0]["exp_dt"]).ToString("d/M/yyyy");
                }
        ...
        {
            ut.Logs("", "HR", "Employee", "fr_mst_employee", "BindControle(emp)", "Exception", ex.Message + ex.InnerException);
            ScriptManager.RegisterStartupScript(Page, Page.GetType(), "al", "sweetAlert('" + ex.Message + ex.InnerException + "','Error to save','warning');", true);
            //ScriptManager.RegisterStartupScript(Page, Page.GetType(), Guid.NewGuid().ToString(), "MsgWarning('File upload failed!','File upload failed');", true);
    ...
    protected void grdVehicle_RowDeleting(object sender, GridViewDeleteEventArgs e)
    {
        ...
        List<cArrayList> arr = new List<cArrayList>();
        arr.Add(new cArrayList("@emp_cd", ddlEmployee.SelectedValue));
