...
public partial class fm_mstdiscount2 : System.Web.UI.Page
{
    ...
    void vCheckAccess()
    {
        int nAccess = bll.nCheckAccess("discount", Request.Cookies["usr_id"].Value.ToString());
        if (nAccess == 0)
        {
    ...
    public static string[] GetCompletionList(string prefixText, int count, string contextKey)
    {
        ...
        while (rs.Read())
        {
            sCust = AjaxControlToolkit.AutoCompleteExtender.CreateAutoCompleteItem(rs["disc_cd"].ToString() + " | " + rs["remark"].ToString() + " | " , rs["disc_cd"].ToString());
            lCust.Add(sCust);
        }
        rs.Close();
