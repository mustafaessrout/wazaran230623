...
public partial class fm_lookupitem : System.Web.UI.Page
{
    cbll bll = new cbll();
    double unitprice = 0;
        ...
        if (!IsPostBack)
        {
            try
            {
                ...
                if (sFreeItemProduct == "I")
                {
                    ...
                    arr.Add(new cArrayList("@salespointcd", Request.Cookies["sp"].Value.ToString()));
                    bll.vBindingGridToSp(ref grdsearch, "sp_tdiscount_freeitem_getbywhs", arr);
                }
                else if (sFreeItemProduct == "G")
                {
                    ...
                    arr.Add(new cArrayList("@so_cd", Request.QueryString["so_cd"].ToString()));
                    arr.Add(new cArrayList("@salespointcd", Request.Cookies["sp"].Value.ToString()));
                    bll.vBindingGridToSp(ref grdsearch, "sp_tdiscount_freeproduct_getbywhs", arr);
                }
            ...
            catch (Exception ex)
            {
                ...
                bll.vHandledError(ref ex, Request.Cookies["sp"].Value.ToString()+" : fm_lookupitem");
    ...
    protected void grdsearch_PageIndexChanging(object sender, GridViewPageEventArgs e)
    {
        try
        {
            ...
            arr.Clear();
            arr.Add(new cArrayList("@disc_cd", Request.QueryString["dc"].ToString()));
            bll.vBindingGridToSp(ref grdsearch, "sp_tdiscount_freeitem_get", arr);
    ...
    protected void btsave_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            foreach (GridViewRow row in grdsearch.Rows)
            {
                ...
                HiddenField hdstock = (HiddenField)row.FindControl("hdstock");
                ...
                {
                    if (Convert.ToDouble(hdstock.Value) > 0)
                    {
                        if (dTot > Convert.ToDouble(hdstock.Value))
                        {
            ...
            foreach (GridViewRow row in grdsearch.Rows)
            {
                ...
                if (txqty.Text != "0")
                {
                    ...
                    else if (Request.QueryString["t"] == "cn")
                    {
                        ...
                        arr.Add(new cArrayList("@bin_cd", cbbin.SelectedValue.ToString()));
                        bll.vInsertWrkCanvasOrderFreeItem(arr);
                    }
                }
            }
    ...
    protected void grdsearch_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        try
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                string sStockCurrent = "0", sStock_conv = "";
                ...
                if (Request.QueryString["t"] == "to")
                {
                    ...
                    // Convert to Multi UOM ; By Cino ; 2020-11-27
                    sStock_conv = bll.vLookUp("select dbo.fn_getqtyconv('" + lbitemcode.Text + "','"+cbuomfree.SelectedValue.ToString()+"'," + sStock + ")");
                ...
                //lbstock.Text = sStock;
                hdstock.Value = sStock;
                lbstock.Text = sStock_conv;


                if (cbuomfree.SelectedValue.ToString() == "PCS")
                {
                    txqty.TextMode = TextBoxMode.Number;
                }
                else
                {
                    txqty.TextMode = TextBoxMode.SingleLine;
                }
            }
        }
        catch (Exception ex)
