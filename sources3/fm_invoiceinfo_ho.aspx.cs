...
public partial class fm_invoiceinfo_ho : System.Web.UI.Page
{
    cbll bll = new cbll();
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            statusFree = bll.vLookUp("select b.inv_no from tsalesorder_discitem_ho a inner join tmst_dosales_ho b on a.so_cd=b.so_cd where b.inv_no='" + sinvNo + "'");
            ...
            string x = bll.vLookUp("select count(inv_no) from twrk_claimconfirm where inv_no='" + sinvNo + "'");
            if (x == "0")
            {
                arr.Add(new cArrayList("@inv_no", sinvNo));
                bll.vInsertTWRKClaimHOConfirm(arr);
            }
            arr.Clear();
            arr.Add(new cArrayList("@inv_no", sinvNo.ToString()));
            bll.vGetMstinvoiceHO(arr, ref rs);
            while (rs.Read())
            {
                lbinvoice.Text = sinvNo;
                lbcust.Text = rs["cust_cd"].ToString() + "-" + rs["cust_nm"].ToString();
                lbotlcd.Text = rs["otlcd"].ToString();
                lbreceiveddt.Text = Convert.ToDateTime(rs["so_dt"].ToString()).ToShortDateString();
                bll.sFormat2ddmmyyyy(ref lbreceiveddt);
                lbinvoicedt.Text = Convert.ToDateTime(rs["so_dt"].ToString()).ToShortDateString();
            ...
            arr.Add(new cArrayList("@inv_no", sinvNo));
            bll.vBindingGridToSp(ref grdinvoice, "sp_invoice_ho_get", arr);
            arr.Clear();
            arr.Add(new cArrayList("@type", "FREE"));
            arr.Add(new cArrayList("@inv_no", sinvNo));
            bll.vBindingGridToSp(ref grdfree, "sp_invoice_ho_get", arr);            
        }
    }
