...
public partial class fm_reqdiscount3 : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            try
            {
                ...
                bll.vBindingComboToSp(ref cbprodcd, "sp_tmst_product_get", "prod_cd", "prod_nm", arr);
                ...
                bll.vBindingComboToSp(ref rdsalespoint, "sp_tmst_salespoint_get", "salespointcd", "salespoint_desc");
                rdsalespoint.Items.Insert(0, new ListItem("<< ALL Branch >>", "-1"));
                rdproduct_SelectedIndexChanged(sender, e);
                rdiproduct_SelectedIndexChanged(sender, e);

                // Additional Feature Maximum Discount used : 2021-11-30
                bll.vBindingFieldValueToCombo(ref cbmaxdisc, "disc_blocked_typ");
                cbmaxdisc.SelectedValue = "NA";
                cbmaxdisc_SelectedIndexChanged(sender, e);
                dtdelivery.Text = Request.Cookies["waz_dt"].Value;
                dtstart.Text = Request.Cookies["waz_dt"].Value;
                dtend.Text = Request.Cookies["waz_dt"].Value;
                dtmaxclaim.Text = Request.Cookies["waz_dt"].Value;
                txregularcost.Text = "0";
                txnetcost.Text = "0";
                cd.v_disablecontrol(dtmaxclaim);
                cd.v_disablecontrol(txnetcost);
                cd.v_disablecontrol(txregularcost);
                cd.v_disablecontrol(txbenefit);

            }
            catch (Exception ex)
            {
                Session["error"] = "500" + " - " + bll.sGetFieldValue("error", "500");
                bll.vHandledError(ref ex, Request.Cookies["sp"].Value.ToString()+" : fm_reqdiscount3");
    ...
    protected void btaddformula_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            double _minqty = 0;
            if (!double.TryParse(txordermin.Text , out _minqty))
            {
                ScriptManager.RegisterStartupScript(Page, Page.GetType(), Guid.NewGuid().ToString(), "sweetAlert('Minimum order must numeric!','minimum order','warning');", true);
                return;
            }
            if (_minqty == 0)
            {
                ScriptManager.RegisterStartupScript(Page, Page.GetType(), Guid.NewGuid().ToString(), "sweetAlert('Min qty must greater than zero!','minimum order','warning');", true);
                return;
            }
            if (cbuom.SelectedValue.ToString() == "")
            {
                ScriptManager.RegisterStartupScript(Page, Page.GetType(), Guid.NewGuid().ToString(), "sweetAlert('UOM can not empty','Formula Discount','warning');", true);
                return;
            }
            if (txordermin.Text.ToString() == "")
            {
                ScriptManager.RegisterStartupScript(Page, Page.GetType(), Guid.NewGuid().ToString(), "sweetAlert('Order Min can not empty','Formula Discount','warning');", true);
                return;
            }
            ...
            if (rdmethod.SelectedValue.ToString() == "FG")
            {
                ...
                {
                    ScriptManager.RegisterStartupScript(Page, Page.GetType(), Guid.NewGuid().ToString(), "sweetAlert('UOM Free can not empty','Formula Discount','warning');", true);
                    return;
                }
                txpercent.Text = "0";
                txcash.Text = "0";
            ...
            else if (rdmethod.SelectedValue.ToString() == "PC")
            {
                txcash.Text = "0";
                txqty.Text = "0";
                //if (Double.Parse(txpercent.Text) == 100)
                //{
                //    ScriptManager.RegisterStartupScript(Page, Page.GetType(), Guid.NewGuid().ToString(), "sweetAlert('Total Percentage must be less than 100%','Formula Discount','warning');", true);
                //    return;
                //}
            }
            arr.Add(new cArrayList("@usr_id", Request.Cookies["usr_id"].Value.ToString()));
    ...
    protected void btrefresh_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            while (rs.Read())
            {
                ...
                //{
                //    txproposal.Text = "";
                //    ScriptManager.RegisterStartupScript(Page, Page.GetType(), Guid.NewGuid().ToString(), "sweetAlert('Sorry, this proposal expired.',' " + rs["prop_no"].ToString() + " ','warning');", true);
                //    return;
                //}      
    ...
    protected void btsave_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            //if (txproposal.Text == "")
            //{
            //    ScriptManager.RegisterStartupScript(Page, Page.GetType(), Guid.NewGuid().ToString(), "sweetAlert('Proposal No can not empty','Proposal No','warning');", true);
            //    return;
            //}

            if (txremark.Text == "")
            {
                ScriptManager.RegisterStartupScript(Page, Page.GetType(), Guid.NewGuid().ToString(), "sweetAlert('Remark can not empty','Remark ','warning');", true);
                return;
            }
            txbenefit.Text = "NA";
            //if (txbenefit.Text == "")
            //{
            //    ScriptManager.RegisterStartupScript(Page, Page.GetType(), Guid.NewGuid().ToString(), "sweetAlert('Benefit Promotion can not empty','Benefit Promotion','warning');", true);
            //    return;
            //}
            if (rdused.SelectedValue.ToString() == "")
            {
                ScriptManager.RegisterStartupScript(Page, Page.GetType(), Guid.NewGuid().ToString(), "sweetAlert('Disc Used can not empty','Disc Used','warning');", true);
                return;
            }
            if (txregularcost.Text == "")
            {
                ScriptManager.RegisterStartupScript(Page, Page.GetType(), Guid.NewGuid().ToString(), "sweetAlert('Regular Cost can not empty','Regular Cost','warning');", true);
                return;
            }
            if (txnetcost.Text == "")
            {
                ScriptManager.RegisterStartupScript(Page, Page.GetType(), Guid.NewGuid().ToString(), "sweetAlert('Net Cost can not empty','Net Cost','warning');", true);
                return;
            }
            if (txminqty.Text == "")
            {
                ScriptManager.RegisterStartupScript(Page, Page.GetType(), Guid.NewGuid().ToString(), "sweetAlert('Minimum Order can not empty','Minimum Order','warning');", true);
                return;
            }
            if (txmaxorder.Text == "")
            {
                ScriptManager.RegisterStartupScript(Page, Page.GetType(), Guid.NewGuid().ToString(), "sweetAlert('Maximum Order can not empty','Maximum Order','warning');", true);
                return;
            }
            if (rdmethod.SelectedValue.ToString() == "")
            {
                ScriptManager.RegisterStartupScript(Page, Page.GetType(), Guid.NewGuid().ToString(), "sweetAlert('Discount Mechanism can not empty','Discount Mechanism','warning');", true);
                return;
            }
            ...
            if (rdmethod.SelectedValue.ToString() == "FG")
            {
                ...
                {
                    ScriptManager.RegisterStartupScript(Page, Page.GetType(), Guid.NewGuid().ToString(), "sweetAlert('Please select free item or product','Free Item or Product','warning');", true);
                    return;
                }
            }
            if (grdformula.Rows.Count == 0)
            {
                ScriptManager.RegisterStartupScript(Page, Page.GetType(), Guid.NewGuid().ToString(), "sweetAlert('Formula Discount can not empty','Formula Discount','warning');", true);
                return;
            }
            ...
            //arr.Add(new cArrayList("@disc_dt", DateTime.ParseExact(DateTime.Now.ToString("dd/MM/yyyy"), "d/MM/yyyy", System.Globalization.CultureInfo.InvariantCulture)));
            if (rdiproduct.SelectedValue.ToString() == "Y")
            { arr.Add(new cArrayList("@rditem", hditem.Value.ToString())); }
            else { arr.Add(new cArrayList("@rditem", rdproduct.SelectedValue.ToString())); }
            ...
            else { arr.Add(new cArrayList("@rdfreeitem", rdfree.SelectedValue.ToString())); }
            //bll.vInsertMstDiscount(arr);
            bll.vInsertMstDiscount(arr, ref sDiscNo);
            ...
            else
            {
                ...
                arr.Add(new cArrayList("@disc_no", sDiscNo));
                arr.Add(new cArrayList("@usr_id", Request.Cookies["usr_id"].Value.ToString()));
                if (rdproduct.SelectedValue.ToString() == "P")
                {
            ...
            arr.Clear();
            if (uplpromotion.FileName != "")
            {
                FileInfo fip = new FileInfo(uplpromotion.FileName);
                string extp = fip.Extension;
                byte[] fsp = uplpromotion.FileBytes;
                if (fsp.Length <= 500000)
                {
                    if (extp == ".jpg" || extp == ".jpeg" || extp == ".bmp" || extp == ".gif" || extp == ".png" || extp == ".pdf" || extp == ".JPEG" || extp == ".JPG" || extp == ".BMP" || extp == ".GIF" || extp == ".PNG" || extp == ".PDF")
                    {
                        if ((uplpromotion.FileName != "") || (uplpromotion.FileName != null))
                        {
                            uplpromotion.SaveAs(bll.sGetControlParameter("image_path") + "/discount_doc/" + sDiscNo + extp);
                            uplpromotion.Visible = false;
                            hpfile_promo.Visible = true;
                            lbfilelocpromo.Text = sDiscNo + extp;
                            hpfile_promo.NavigateUrl = "/images/discount_doc/" + sDiscNo + extp;
                            arr.Add(new cArrayList("@disc_cd", sDiscNo));
                            arr.Add(new cArrayList("@doc_cd", "P37"));
                            arr.Add(new cArrayList("@filename", sDiscNo + extp));
            ...
            // Maximum Amount / Qty for discount
            if (cbmaxdisc.SelectedValue.ToString() != "NA")
            {
                arr.Clear();
                arr.Add(new cArrayList("@usr_id", Request.Cookies["usr_id"].Value.ToString()));
                arr.Add(new cArrayList("@disc_no", sDiscNo));
                arr.Add(new cArrayList("@is_blocked", "1"));
                arr.Add(new cArrayList("@blocked_typ", cbmaxdisc.SelectedValue.ToString()));
                arr.Add(new cArrayList("@amount", txmaxamount.Text));
                arr.Add(new cArrayList("@uom", cbmaxuom.SelectedValue.ToString()));
                arr.Add(new cArrayList("@amount_typ",  (cbmaxdisc.SelectedValue.ToString()=="SV" ? "V" : "Q") ));
                bll.vInsertDiscountMaximum(arr);
            }
            ...
            //{
            //    //ScriptManager.RegisterStartupScript(Page, Page.GetType(), Guid.NewGuid().ToString(), "sweetAlert('No Image uploaded','Please scan the document and upload','warning');", true);
            //    //return;
            ...
            ScriptManager.RegisterStartupScript(Page, Page.GetType(), Guid.NewGuid().ToString(), "sweetAlert('New Discount has been created','" + sDiscNo + "','success');", true);
    ...
    protected void btaddcust_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (rdcust.SelectedValue.ToString() == "C")
            {
                if (hdcust.Value.ToString() == "" && hdcust_new.Value.ToString() == "")
                {
                    ScriptManager.RegisterStartupScript(Page, Page.GetType(), Guid.NewGuid().ToString(), "sweetAlert('Customer Not yet selcted','select customer','warning');", true);
                    return;
                }
            }
            List<cArrayList> arr = new List<cArrayList>();
            if (rdcust.SelectedValue.ToString() == "C")
            {
                string[] cust = hdcust_new.Value.ToString().Split('_');
                arr.Add(new cArrayList("@usr_id", Request.Cookies["usr_id"].Value.ToString()));
                arr.Add(new cArrayList("@cust_cd", cust[1].ToString()));
                //arr.Add(new cArrayList("@cust_nm", txsearchcust.Text.ToString()));
                arr.Add(new cArrayList("@salespointcd", cust[0].ToString()));
    ...
    public static string[] GetCompletionList2(string prefixText, int count, string contextKey)
    {
        ...
        arr.Add(new cArrayList("@usr_id", HttpContext.Current.Request.Cookies["usr_id"].Value.ToString()));
        bll.vSearchMstCustomerDisc(arr, ref rs);
        while (rs.Read())
        {
            sCust = AjaxControlToolkit.AutoCompleteExtender.CreateAutoCompleteItem(rs["salespointcd"].ToString() + " | " + rs["cust_cd"].ToString() + "-" + rs["cust_nm"].ToString(), rs["salespointcd"].ToString()+"_"+rs["cust_cd"].ToString());
            lCust.Add(sCust);
        }
        rs.Close();
    ...
    protected void grditem_RowDeleting(object sender, GridViewDeleteEventArgs e)
    {
        try
        {
            ...
            List<cArrayList> arr = new List<cArrayList>();
            if (rdiproduct.SelectedValue.ToString() == "N")
            {
                arr.Clear();
                Label lbitemcode = (Label)grdviewitem.Rows[e.RowIndex].FindControl("lbitemcode");
                arr.Add(new cArrayList("@usr_id", Request.Cookies["usr_id"].Value.ToString()));
                arr.Add(new cArrayList("@item_cd", lbitemcode.Text));
                bll.vDelWrkItem(arr); arr.Clear();
                arr.Add(new cArrayList("@usr_id", Request.Cookies["usr_id"].Value.ToString()));
                bll.vBindingGridToSp(ref grdviewitem, "sp_twrk_item_get", arr);
                grdviewgroup.Attributes.Add("style", "display:none");
                grdviewitem.Attributes.Remove("style");
                txsearchitem.Text = "";
            }
            else
            {
                ...
                bll.vBindingGridToSp(ref grdviewitem, "sp_tproposal_item_get", arr);
                //bll.vBindingGridToSp(ref grdviewitem, "sp_discount_item_get", arr);
                //ScriptManager.RegisterStartupScript(Page, Page.GetType(), Guid.NewGuid().ToString(), "sweetAlert('Item Deleted','Deleted Item','warning');", true);
                //return;
            }
        }
        catch (Exception ex)
    ...
    protected void grdproduct_RowDeleting(object sender, GridViewDeleteEventArgs e)
    {
        try
        {
            ...
            if (rdiproduct.SelectedValue.ToString() == "N")
            {
                arr.Clear();
                arr.Add(new cArrayList("@usr_id", Request.Cookies["usr_id"].Value.ToString()));
                arr.Add(new cArrayList("@prod_cd", cbprodcd.SelectedValue.ToString()));
                bll.vDelWrkProduct(arr); arr.Clear();
                arr.Add(new cArrayList("@usr_id", Request.Cookies["usr_id"].Value.ToString()));
                bll.vBindingGridToSp(ref grdcusgrcd, "sp_twrk_product_get", arr);
                grdviewitem.Attributes.Add("style", "display:none");
                grdviewgroup.Attributes.Remove("style");
            }
            else
            {
                ...
                arr.Add(new cArrayList("@usr_id", Request.Cookies["usr_id"].Value.ToString()));
                bll.vBindingGridToSp(ref grdviewgroup, "sp_tproposal_productgroup_get", arr);
                //ScriptManager.RegisterStartupScript(Page, Page.GetType(), Guid.NewGuid().ToString(), "sweetAlert('Product Deleted','Deleted Product','warning');", true);
                //return;
            }
        }
        catch (Exception ex)
    ...
    protected void btaddsalespoint_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                ScriptManager.RegisterStartupScript(Page, Page.GetType(), Guid.NewGuid().ToString(), "sweetAlert('Salespoint Not yet selcted','select Salespoint','warning');", true);
                return;
            }
            else
            {
    ...
    protected void rdiproduct_SelectedIndexChanged(object sender, EventArgs e)
    {
        if (rdiproduct.SelectedValue.ToString() == "N")
        {
            rdproduct.Attributes.Remove("style");
            rdproduct_SelectedIndexChanged(sender, e);
            btaddproduct.Attributes.Remove("style");
            grdviewitem.Visible = false;
            grdviewgroup.Visible = false;
        }
        else
        {
            rdproduct.Attributes.Add("style", "display:none");
            txsearchitem.Attributes.Add("style", "display:none");
            cbprodcd.Attributes.Add("style", "display:none");
            btaddproduct.Attributes.Add("style", "display:none");
            grdviewitem.Attributes.Add("style", "display:none");
            grdviewgroup.Attributes.Add("style", "display:none");
            if (rdproduct.SelectedValue.ToString() == "P")
            {
    ...
    protected void rdproduct_SelectedIndexChanged(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                txsearchitem.Attributes.Add("style", "display:none");
                cbprodcd.Attributes.Remove("style");
            }
        }
        catch (Exception ex)
        {
    ...
    protected void btaddproduct_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (rdproduct.SelectedValue.ToString() == "P")
            {
                if (hditem_new.Value.ToString() == "")
                {
                    ScriptManager.RegisterStartupScript(Page, Page.GetType(), Guid.NewGuid().ToString(), "sweetAlert('Item Not yet selcted','select item','warning');", true);
                    return;
                }
            }
            List<cArrayList> arr = new List<cArrayList>();
            ...
            {
                arr.Add(new cArrayList("@usr_id", Request.Cookies["usr_id"].Value.ToString()));
                arr.Add(new cArrayList("@item_cd", hditem_new.Value));
                bll.vInsertWrkItem(arr); arr.Clear();
                arr.Add(new cArrayList("@usr_id", Request.Cookies["usr_id"].Value.ToString()));
                bll.vBindingGridToSp(ref grdviewitem, "sp_twrk_item_get", arr);
                grdviewgroup.Attributes.Add("style", "display:none");
            ...
            else
            {
                ...
                bll.vInsertWrkProduct(arr); arr.Clear();
                arr.Add(new cArrayList("@usr_id", Request.Cookies["usr_id"].Value.ToString()));
                bll.vBindingGridToSp(ref grdviewgroup, "sp_twrk_product_get", arr);
                grdviewitem.Attributes.Add("style", "display:none");
                grdviewgroup.Attributes.Remove("style");
                grdviewgroup.Visible = true;
            }
    ...
    protected void grdslspointnew_PageIndexChanging(object sender, GridViewPageEventArgs e)
    {
        List<cArrayList> arr = new List<cArrayList>();
        grdfreeitem.PageIndex = e.NewPageIndex;
        arr.Add(new cArrayList("@usr_id", Request.Cookies["usr_id"].Value.ToString()));
    ...
    protected void cbmaxdisc_SelectedIndexChanged(object sender, EventArgs e)
    {
        if (cbmaxdisc.SelectedValue.ToString() == "NA")
        {
            vMaxDiscount.Attributes.Add("style", "display:none");
        }
        else
        {
            vMaxDiscount.Attributes.Remove("style");
            cbmaxuom.SelectedValue = (cbmaxdisc.SelectedValue.ToString() == "SV") ? "SAR" : "CTN";
        }
    }

    protected void grdslspoint_RowDeleting(object sender, GridViewDeleteEventArgs e)
    {
        List<cArrayList> arr = new List<cArrayList>();
        Label lbsalespoint = (Label)grdslspoint.Rows[e.RowIndex].FindControl("lblslspointcd");
        string _sql = "delete twrk_salespoint where usr_id='" + Request.Cookies["usr_id"].Value +"' " +
            "and salespointcd='"+lbsalespoint.Text+"'";
    ...
    protected void grdslspointnew_RowDeleting(object sender, GridViewDeleteEventArgs e)
    {
        List<cArrayList> arr = new List<cArrayList>();
        Label lbsalespoint = (Label)grdslspointnew.Rows[e.RowIndex].FindControl("lblslspointcd");
        string _sql = "delete twrk_salespoint where usr_id='" + Request.Cookies["usr_id"].Value + "' " +
            "and salespointcd='" + lbsalespoint.Text + "'";
        bll.vExecuteSQL(_sql);
