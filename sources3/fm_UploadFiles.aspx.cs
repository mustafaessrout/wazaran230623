...
public partial class fm_UploadFiles : System.Web.UI.Page
{
    ...
    protected void btnUpload_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            string baseName =  HttpContext.Current.Server.MapPath("/admin/ImageFolder/") + fileName;
            //string fileLocation = Server.MapPath(baseName);
            ...
            if (upl.HasFile)
            {
                ...
                if (fs.Length <= 6000000)
                {
                    ...
                    {
                        Session["fileSize"] = fs;
                        Session["fileName"] = Request.Cookies["sp"].Value.ToString() + "_" + Convert.ToString(upl.FileName);
                        ...
                        //Session["fileLocation"] = bll.sGetControlParameter("image_path") + "/account/asset/" + Request.Cookies["sp"].Value.ToString() + "-" + Convert.ToString(upl.FileName);
                        Session["fileLocation"] = bll.sGetControlParameter(imagePath) + Request.Cookies["sp"].Value.ToString() + "-" + Convert.ToString(upl.FileName);
