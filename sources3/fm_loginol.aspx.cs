...
public partial class fm_loginol : System.Web.UI.Page
{
    cbll bll = new cbll();
    ...
    protected void btlogin_Click(object sender, EventArgs e)
    {
            ...
            if (txusername.Text != "" && txpassword.Text != "")
            {
                while (rs.Read())
                {
                    ...
                    if (usr == "0" && pass != "-1")
                    {
                        ...
                        totSP = int.Parse(bll.vLookUp("select count(*) from tuser_salespoint where usr_id='"+ txusername.Text + "'"));
                        sSalespoint =  ((totSP == 1) ? rs["salespointcd"].ToString() : "0");
                        sEmpCode = rs["emp_cd"].ToString();
                        sEmail = rs["email"].ToString();
                        sPwdExpDt = rs["pwd_exp_dt"].ToString();
                        sLogo = rs["logo"].ToString();
                    }
		...
		Response.Cookies["logo"].Value = sLogo;
                ...
                else
                {
                    ...
                    arr.Clear();
                    cInfo = 1; //Get always no check info IAG, 9 Jul 2022
                    if (cInfo == 0)
                    {
                    ...
                    else if (cInfo > 0)
                    {
                        ...
                        if (sSalespoint != "0")
                        {
                            Response.Cookies["sp"].Value = sSalespoint; // Move after switcher 
                            ...
                            ScriptManager.RegisterStartupScript(Page, Page.GetType(), Guid.NewGuid().ToString(), "HideProgress();", true);
                            Response.Redirect("default.aspx", false); //for Branch
                        }
                        else
                        {
                            ScriptManager.RegisterStartupScript(Page, Page.GetType(), Guid.NewGuid().ToString(), "HideProgress();", true);
                            Response.Redirect("switcherol.aspx", false); // for HO
    ...
    protected void btcontinue_Click(object sender, EventArgs e)
    {
        ...
        Session["allaspfile"] = arrallaspx;
        Session["allmenucaption"] = arrallmenucaption;
        //Response.Redirect("default.aspx"); // for direct to branch
        Response.Redirect("switcherol.aspx");
