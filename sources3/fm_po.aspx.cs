...
public partial class fm_po : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            try
            {
                ...
                arr.Clear();
                arr.Add(new cArrayList("@fld_nm", "po_order_typ"));
                arr.Add(new cArrayList("@hiddendata", 0));
                bll.vBindingFieldValueToCombo(ref cbordertype, arr);
                ...
                if (Request.QueryString["po"] != null)
                {
                    ...
                    while (rs.Read())
                    {
                        ...
                        cbordertype.SelectedValue = rs["order_typ"].ToString();
    ...
    private void bindinggrdloading()
    {
        try
        {
            ...
            if (grdloading.Rows.Count > 0)
            {
                ...
                lbtotqty.Text = bll.vLookUp("select sum(b.qty) from tinternal_transfer a inner join tinternal_transfer_dtl b on a.trf_no=b.trf_no and a.salespointcd=b.salespointcd where a.sta_id in ('A','W') and a.trf_typ = 'I' and a.salespointcd='" + cbsalespoint.SelectedValue.ToString() + "'");
            }
        }
        catch (Exception ex)
        {
    ...
    protected void btsave_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            string sto_nm = "";
            string typeCustomer = "";
            if (cbtype.SelectedValue.ToString().Trim() == "CU") { typeCustomer = bll.vLookUp("select cuscate_cd from tmst_customer where cust_cd='" + hdcust.Value.ToString() + "' and salespointcd='" + cbsalespoint.SelectedValue.ToString() + "'"); sto_nm = txsearch.Text; } else { sto_nm = cbmaster.SelectedItem.Text; }
            if (txpono.Text == "" || txpono.Text == "NEW") { po_no = Request.Cookies["usr_id"].Value.ToString(); } else { po_no = txpono.Text; }
            ...
            if (txpono.Text == "NEW" || txpono.Text == "")
            {
                ...
                arr.Add(new cArrayList("@po_order_typ", cbordertype.SelectedValue.ToString()));
    ...
    protected void bttmp3_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            cbsalespoint.SelectedValue = Convert.ToString(Session["salespoint"]);
            List<cArrayList> arr = new List<cArrayList>();
            arr.Add(new cArrayList("@po_no", txpono.Text));
            arr.Add(new cArrayList("@salespointcd", cbsalespoint.SelectedValue.ToString()));
            bll.vGetMstPO(ref rs, arr);
    ...
    protected void btadd_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                if (bll.vLookUp("select dbo.fn_checkcustblocked('" + hdcust.Value.ToString() + "','S','" + cbsalespoint.SelectedValue.ToString() + "')") == "ok")
                {
                    double dmaxTransaction = Double.Parse(bll.vLookUp("select dbo.fn_getmaxtransactionamt('" + hdcust.Value.ToString() + "','"+ cbsalespoint.SelectedValue.ToString() + "')"));
                    double doverdue = Double.Parse(bll.vLookUp("select isnull(dbo.fn_getoverdue('" + hdcust.Value.ToString() + "','"+ cbsalespoint.SelectedValue.ToString() + "'),0)"));
                    if (doverdue > 0)
                    {
    ...
    private void bindinggrd()
    {
        try
        {
            ...
            typeCustomer = bll.vLookUp("select cuscate_cd from tmst_customer where cust_cd='" + hdcust.Value.ToString() + "' and salespointcd='" + cbsalespoint.SelectedValue.ToString() + "'");
            if (txpono.Text == "" || txpono.Text == "NEW") { po_no = Request.Cookies["usr_id"].Value.ToString(); } else { po_no = txpono.Text; }
            List<cArrayList> arr = new List<cArrayList>();
            arr.Add(new cArrayList("@po_no", po_no));
            arr.Add(new cArrayList("@salespointcd", cbsalespoint.SelectedValue.ToString()));
            ...
            if (grdpo.Rows.Count > 0)
            {
                ...
                Label lbtotsubtotal = (Label)grdpo.FooterRow.FindControl("lbtotsubtotal");
                lbtotqty.Text = bll.vLookUp("select sum(qty) from tpo_dtl where po_no='" + po_no + "' and salespointcd='" + cbsalespoint.SelectedValue.ToString() + "'");
                lbtotsubtotal.Text = bll.vLookUp("select sum(subtotal) from tpo_dtl where po_no='" + po_no + "' and salespointcd='" + cbsalespoint.SelectedValue.ToString() + "'");
            }
    ...
    protected void btprint_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            lFormula.Add("{tmst_po.salespointcd} = '" + cbsalespoint.SelectedValue.ToString() + "'");
    ...
    protected void btstock_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            //else
            //{
            txstock.Text = bll.vLookUp("select round(dbo.sfnGetStockBookingAll('" + cbsalespoint.SelectedValue.ToString() + "','" + hditem.Value + "','" + cbmaster.SelectedValue.ToString() + "'),0)");
            hdstock.Value = bll.vLookUp("select round(dbo.sfnGetStockBookingAll('" + cbsalespoint.SelectedValue.ToString() + "','" + hditem.Value + "','" + cbmaster.SelectedValue.ToString() + "'),0)");
            //txqty.Focus();
            ScriptManager.RegisterStartupScript(Page, Page.GetType(), "set", "$('#txqty').val('" + bll.vLookUp("select round(dbo.sfnGetStockBookingAll('" + cbsalespoint.SelectedValue.ToString() + "','" + hditem.Value + "','" + cbmaster.SelectedValue.ToString() + "'),0)") + "')", true);
            //    ScriptManager.RegisterStartupScript(Page, Page.GetType(), "al", "sweetAlert('Attention','Transaction has been complete, data can not save','error');", true);
            //    return;
            //}
        }
